...
import { GlobalSearchState } from "../../global-search/models/global-search.state";
...
import { ContactDto, ContactLocationDto } from "../../_data-services/_dtos/contact.dto";
...
export class AddressBookStateService {
    ...
    constructor(private contactsDataService: ContactsDataService,
                ...
                private searchHelperService: GlobalSearchHelperService) {
        ...
        this.searchStateService.state$
            .subscribe(this.onSearchStateUpdated)
    }
    ...
    private onGetInitialContactsForUserSuccess = (response: RandomUserResponseDto) => {
    ...
    private onSearchStateUpdated = (updatedState: GlobalSearchState) => {
        this.state.searchTerm = updatedState.searchValue;

        this.state.filteredContacts = this.filterContacts(this.state.searchTerm, this.state.contacts);
    ...
    private filterContacts = (searchTerm: string, contacts: Contact[]) => {
        if (searchTerm) {
            return contacts.filter(contact => { 
                const returnValue = this.searchHelperService.filterObjectBySearchTerm(contact, this.state.searchTerm);
                return returnValue;
            });
        } else {
            return contacts;
    ...
    private handleLoadNextContactsAction = () => {
        if (!this.isLoading) {
            ...
            this.contactsDataService.getAllContactsForUser(null)
                .pipe(
                    take(1)
    ...
    private onGetMoreContactsSuccess = (response: RandomUserResponseDto) => {
        this.state.contacts = this.state.contacts.concat(response.results.map(this.mapContactDtoToContact));
        this.state.filteredContacts = this.filterContacts(this.state.searchTerm, this.state.contacts);
        this.isLoading = false;
        this.publishStateUpdate();
        this.publishIsLoadingUpdate()
    ...
    private mapContactDtoToContact = (dto: ContactDto) => {
        return <Contact> {
            ...
            cellNumber: dto.cell,
            email: dto.email,
            birthday: new Date(dto.dob.date).toLocaleDateString("en-US", APP_CONSTS.BIRTHDAY_LONG_DATE_FORMAT_OPTIONS),
